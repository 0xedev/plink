        ...
        {#if open}
          <div {...wrapperProps}>
            <div {...props} transition:flyAndScale>
              <p class="text-sm font-medium text-purple-200">Add money</p>
              <div class="flex gap-2">
                {#each addMoneyAmounts as amount}
                  <button
                    onclick={() => ($balance += amount)}
                    class="touch-manipulation rounded-md bg-green-500 px-3 py-2 text-sm font-semibold text-purple-900 transition-colors hover:bg-purple-400 active:bg-purple-600 disabled:bg-neutral-600 disabled:text-neutral-400"
                  >
                    +${amount}
                  </button>
                {/each}
